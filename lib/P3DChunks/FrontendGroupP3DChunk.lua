...
P3D.FrontendGroupP3DChunk = setmetatable(P3D.P3DChunk:newChildClass(P3D.Identifiers.Frontend_Group), {__call = new})
P3D.FrontendGroupP3DChunk.new = new
function P3D.FrontendGroupP3DChunk:parse(Contents, Pos, DataLength)
	local chunk = self.parentClass.parse(self, Contents, Pos, DataLength, self.Identifier)
	
	chunk.Name, chunk.Version, chunk.Alpha = string_unpack("<s1II", chunk.ValueStr)
...
function P3D.FrontendGroupP3DChunk:__tostring()
	...
	return string_pack("<IIIs1II", self.Identifier, headerLen, headerLen + #chunkData, Name, self.Version, self.Alpha) .. chunkData
end