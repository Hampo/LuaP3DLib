...
P3D.QuaternionChannelP3DChunk = setmetatable(P3D.P3DChunk:newChildClass(P3D.Identifiers.Quaternion_Channel), {__call = new})
P3D.QuaternionChannelP3DChunk.new = new
function P3D.QuaternionChannelP3DChunk:parse(Contents, Pos, DataLength)
	...
	for i=1,numFrames do
		local value = {}
		value.W, value.X, value.Y, value.Z, pos = string_unpack("<ffff", chunk.ValueStr, pos)
...
function P3D.QuaternionChannelP3DChunk:__tostring()
	...
	for i=1,framesN do
		...
		values[i] = string_pack("<ffff", value.W, value.X, value.Y, value.Z)
	end
