...
function P3D.TreeP3DChunk:parse(Contents, Pos, DataLength)
	...
	chunk.Minimum = {}
	chunk.Maximum = {}
	num, chunk.Minimum.X, chunk.Minimum.Y, chunk.Minimum.Z, chunk.Maximum.X, chunk.Maximum.Y, chunk.Maximum.Z = string_unpack("<Iffffff", chunk.ValueStr)
...
function P3D.TreeP3DChunk:__tostring()
	...
	return string_pack("<IIIIffffff", self.Identifier, headerLen, headerLen + #chunkData, chunksN, self.Minimum.X, self.Minimum.Y, self.Minimum.Z, self.Maximum.X, self.Maximum.Y, self.Maximum.Z) .. chunkData
end