...
P3D.FrontendMultiTextP3DChunk = setmetatable(P3D.P3DChunk:newChildClass(P3D.Identifiers.Frontend_Multi_Text), {__call = new})
...
function P3D.FrontendMultiTextP3DChunk:parse(Contents, Pos, DataLength)
...
function P3D.FrontendMultiTextP3DChunk:__tostring()
	...
	return string_pack("<IIIs1IiiIIIIBBBBIfs1BBBBBiiI", self.Identifier, headerLen, headerLen + #chunkData, Name, self.Version, self.Position.X, self.Position.Y, self.Dimension.X, self.Dimension.Y, self.Justification.X, self.Justification.Y, self.Colour.B, self.Colour.G, self.Colour.R, self.Colour.A, self.Translucency, self.RotationValue, TextStyleName, self.ShadowEnabled, self.ShadowColour.B, self.ShadowColour.G, self.ShadowColour.R, self.ShadowColour.A, self.ShadowOffset.X, self.ShadowOffset.Y, self.CurrentText) .. chunkData
end