...
P3D.EntityChannelP3DChunk = setmetatable(P3D.P3DChunk:newChildClass(P3D.Identifiers.Entity_Channel), {__call = new})
...
function P3D.EntityChannelP3DChunk:parse(Contents, Pos, DataLength)
	...
	chunk.Values = table_pack(string_unpack("<" .. string.rep("s1", numFrames), chunk.ValueStr, pos))
...
function P3D.EntityChannelP3DChunk:__tostring()
	...
	local values = {}
	for i=1,framesN do
		values[i] = string_pack("<s1", P3D.MakeP3DString(self.Values[i]))
	end
