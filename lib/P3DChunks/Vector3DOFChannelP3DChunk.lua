...
function P3D.Vector3DOFChannelP3DChunk:parse(Endian, Contents, Pos, DataLength)
	local chunk = self.parentClass.parse(self, Endian, Contents, Pos, DataLength, self.Identifier)
	
	local num, pos
	chunk.Version, chunk.Param, num, pos = string_unpack(Endian .. "Ic4I", chunk.ValueStr)
	if Endian == ">" then
	...
	for i=1,num do
		local value = {}
		value.X, value.Y, value.Z, pos = string_unpack(Endian .. "fff", chunk.ValueStr, pos)
...
function P3D.Vector3DOFChannelP3DChunk:__tostring()
	...
	for i=1,num do
		...
		values[num + i] = string_pack(self.Endian .. "fff", value.X, value.Y, value.Z)
	end
	
	local headerLen = 12 + 4 + 4 + 4 + num * 2 + num * 12
	return string_pack(self.Endian .. "IIIIc4I" .. string_rep("H", num) .. string_rep("c12", num), self.Identifier, headerLen, headerLen + #chunkData, self.Version, self.Param, num, table_unpack(values)) .. chunkData
end