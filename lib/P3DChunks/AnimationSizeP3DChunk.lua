...
P3D.AnimationSizeP3DChunk = setmetatable(P3D.P3DChunk:newChildClass(P3D.Identifiers.Animation_Size), {__call = new})
P3D.AnimationSizeP3DChunk.new = new
function P3D.AnimationSizeP3DChunk:parse(Contents, Pos, DataLength)
	local chunk = self.parentClass.parse(self, Contents, Pos, DataLength, self.Identifier)
	
	chunk.Version, chunk.PC, chunk.PS2, chunk.XBOX, chunk.GC = string_unpack("<IIIII", chunk.ValueStr)
...
function P3D.AnimationSizeP3DChunk:__tostring()
	...
	return string_pack("<IIIIIIII", self.Identifier, headerLen, headerLen + #chunkData, self.Version, self.PC, self.PS2, self.XBOX, self.GC) .. chunkData
end