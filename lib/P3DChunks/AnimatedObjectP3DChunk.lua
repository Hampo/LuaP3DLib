...
P3D.AnimatedObjectP3DChunk = setmetatable(P3D.P3DChunk:newChildClass(P3D.Identifiers.Animated_Object), {__call = new})
P3D.AnimatedObjectP3DChunk.new = new
function P3D.AnimatedObjectP3DChunk:parse(Contents, Pos, DataLength)
...
function P3D.AnimatedObjectP3DChunk:__tostring()
	...
	return string_pack("<IIIIs1s1I", self.Identifier, headerLen, headerLen + #chunkData, self.Version, Name, FactoryName, self.StartingAnimation) .. chunkData
end