...
assert(P3D.IntersectP3DChunk == nil, "Chunk type already loaded.")
...
function P3D.IntersectP3DChunk:parse(Endian, Contents, Pos, DataLength)
	...
	num, pos = string_unpack(Endian .. "I", chunk.ValueStr, pos)
	chunk.Positions = {}
	for i=1,num do
		local Position = {}
...
function P3D.IntersectP3DChunk:__tostring()
	...
	return string_pack(self.Endian .. "IIII" .. string_rep("I", indicesN) .. "Ic" .. positionsLength .. "Ic" .. normalsLength, self.Identifier, headerLen, headerLen + #chunkData, indicesN, table_unpack(values)) .. chunkData
end