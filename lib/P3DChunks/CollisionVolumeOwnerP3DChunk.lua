...
assert(P3D.CollisionVolumeOwnerP3DChunk == nil, "Chunk type already loaded.")
...
function P3D.CollisionVolumeOwnerP3DChunk:parse(Endian, Contents, Pos, DataLength)
...
function P3D.CollisionVolumeOwnerP3DChunk:__tostring()
	...
	return string_pack(self.Endian .. "IIII", self.Identifier, headerLen, headerLen + #chunkData, self:GetNumNames()) .. chunkData
end