...
P3D.ImageP3DChunk = setmetatable(P3D.P3DChunk:newChildClass(P3D.Identifiers.Image), {__call = new})
...
function P3D.ImageP3DChunk:parse(Contents, Pos, DataLength)
	local chunk = self.parentClass.parse(self, Contents, Pos, DataLength, self.Identifier)
	
	chunk.Name, chunk.Version, chunk.Width, chunk.Height, chunk.Bpp, chunk.Palettized, chunk.HasAlpha, chunk.Format = string_unpack("<s1IIIIIII", chunk.ValueStr)
...
function P3D.ImageP3DChunk:__tostring()
	...
	return string_pack("<IIIs1IIIIIII", self.Identifier, headerLen, headerLen + #chunkData, Name, self.Version, self.Width, self.Height, self.Bpp, self.Palettized, self.HasAlpha, self.Format) .. chunkData
end