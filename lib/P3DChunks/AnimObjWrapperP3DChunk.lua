...
local function new(self, Name, Unknown, Unknown2)
	assert(type(Name) == "string", "Arg #1 (Name) must be a string")
	assert(type(Unknown) == "number", "Arg #2 (Unknown) must be a number")
	assert(type(Unknown2) == "number", "Arg #3 (Unknown2) must be a number")
...
function P3D.AnimObjWrapperP3DChunk:parse(Contents, Pos, DataLength)
	...
	chunk.Name, chunk.Unknown, chunk.Unknown2 = string_unpack("<s1BB", chunk.ValueStr)
...
function P3D.AnimObjWrapperP3DChunk:__tostring()
	...
	return string_pack("<IIIs1BB", self.Identifier, headerLen, headerLen + #chunkData, Name, self.Unknown, self.Unknown2) .. chunkData
end