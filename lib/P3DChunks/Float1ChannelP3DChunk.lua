...
function P3D.Float1ChannelP3DChunk:parse(Endian, Contents, Pos, DataLength)
	...
	chunk.Values = {string_unpack(Endian .. string_rep("f", numFrames), chunk.ValueStr, pos)}
	chunk.Values[numFrames + 1] = nil
	
	return chunk
end

function P3D.Float1ChannelP3DChunk:__tostring()
	local chunks = {}
	...
	return string_pack(self.Endian .. "IIIIc4I" .. string_rep("H", framesN) .. string_rep("f", framesN), self.Identifier, headerLen, headerLen + #chunkData, self.Version, self.Param, framesN, table_unpack(values)) .. chunkData
end