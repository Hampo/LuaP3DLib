...
if not Settings.ScaleProps then
	return
end

print("Scaling: " .. Path)
local GamePath = GetGamePath(Path)
if WildcardMatch(Path, "art/missions/level0?/cards.p3d", true, true) then
	GamePath = "/GameData/art/cards.p3d"
end
local P3DFile = P3D.P3DFile(GamePath)
local ScaleMultiplier = Settings["L"..Level.."PropScaleMultiplier"]
...
local function ProcessOldBillboardQuadGroup(OldBillboardQuadGroupChunk)
	for chunk in OldBillboardQuadGroupChunk:GetChunks(P3D.Identifiers.Old_Billboard_Quad) do
		...
		chunk.Translation.Y = chunk.Translation.Y + chunk.Height * -ScaleMultiplier
	end
end

local function ProcessOldPrimitiveGroup(OldPrimitiveGroupChunk)
	for chunk in OldPrimitiveGroupChunk:GetChunks(P3D.Identifiers.Position_List) do
...
for chunk in P3DFile:GetChunks(P3D.Identifiers.Anim_Dyna_Phys) do
	for chunk2 in chunk:GetChunks(P3D.Identifiers.Anim_Obj_Wrapper) do
		for chunk3 in chunk2:GetChunks(P3D.Identifiers.Animation) do
			ProcessAnimation(chunk3)
		end
		
		for chunk3 in chunk2:GetChunks(P3D.Identifiers.Skeleton) do
			ProcessSkeleton(chunk3)
		end
		
		for chunk3 in chunk2:GetChunks(P3D.Identifiers.Old_Billboard_Quad_Group) do
			ProcessOldBillboardQuadGroup(chunk3)
		end
		
		for chunk3 in chunk2:GetChunks(P3D.Identifiers.Mesh) do
			ProcessMesh(chunk3)
		end
		
		for chunk3 in chunk2:GetChunks(P3D.Identifiers.Physics_Object) do
			ProcessPhysicsObject(chunk3)
		end
		
		for chunk3 in chunk2:GetChunks(P3D.Identifiers.Collision_Object) do
			ProcessCollisionObject(chunk3)
		end
	end
end

for chunk in P3DFile:GetChunks(P3D.Identifiers.Dyna_Phys) do
	for chunk2 in chunk:GetChunks(P3D.Identifiers.Mesh) do
		ProcessMesh(chunk2)
	end
	
	for chunk2 in chunk:GetChunks(P3D.Identifiers.Physics_Object) do
		ProcessPhysicsObject(chunk2)
	end
	
	for chunk2 in chunk:GetChunks(P3D.Identifiers.Collision_Object) do
		ProcessCollisionObject(chunk2)
	end
end

for chunk in P3DFile:GetChunks(P3D.Identifiers.Breakable_Object) do
	for chunk2 in chunk:GetChunks(P3D.Identifiers.Animation) do
		ProcessAnimation(chunk2)
	end
	
	for chunk2 in chunk:GetChunks(P3D.Identifiers.Skeleton) do
		ProcessSkeleton(chunk2)
	end
	...
	for chunk2 in chunk:GetChunks(P3D.Identifiers.Particle_System_Factory) do
		ProcessParticleSystemFactory(chunk2)
	end
end

for chunk in P3DFile:GetChunks(P3D.Identifiers.Locator) do
	if chunk.Type == 9 then
		if chunk.ActionName == "SummonVehiclePhone" then
			local height = 2.5 * -ScaleMultiplier
			for chunk2 in chunk:GetChunks(P3D.Identifiers.Trigger_Volume) do
				chunk2.HalfExtents.X = chunk2.HalfExtents.X * ScaleMultiplier
				chunk2.HalfExtents.Y = chunk2.HalfExtents.Y * ScaleMultiplier
				chunk2.HalfExtents.Z = chunk2.HalfExtents.Z * ScaleMultiplier
			end
			chunk.Position.Y = chunk.Position.Y + height
		elseif chunk.ActionName == "CollectorCard" or chunk.ActionName == "DestroyObject" then
			for chunk2 in chunk:GetChunks(P3D.Identifiers.Trigger_Volume) do
				chunk2.HalfExtents.X = chunk2.HalfExtents.X * ScaleMultiplier
				chunk2.HalfExtents.Y = chunk2.HalfExtents.Y * ScaleMultiplier
				chunk2.HalfExtents.Z = chunk2.HalfExtents.Z * ScaleMultiplier
